...
# Point to the current folder (where validate_logs.py lives)
DATA_FOLDER = Path("/Users/melinaahmad/Library/Mobile Documents/com~apple~CloudDocs/Shortcuts/New")
ALL_LISTS_FILE = DATA_FOLDER / "all_lists.json"
...
# -----------------------------
if ALL_LISTS_FILE.exists():
    with open(ALL_LISTS_FILE, "r", encoding="utf-8") as f:
        all_lists = json.load(f)
else:
    st.error("⚠️ all_lists.json not found in repo")
    all_lists = {}

ALL_CONDITIONS = all_lists.get("conditions", [])
NUTRITION_OPTIONS = all_lists.get("nutrition", [])
...
def save_log(date, data):
    ...
    filepath = DATA_FOLDER / filename
    data["validated"] = True
    restored_data = restore_emojis_and_times(data)
    with open(filepath, "w", encoding="utf-8") as f:
        json.dump(restored_data, f, ensure_ascii=False, separators=(",", ":"))
...
def build_minute_timeline(entries, selected_date, prev_last_entry="⚪️", universal_start_min=None):
    ...
    entries_sorted = sorted(entries, key=lambda e: parse_datetime_safe(e["time"]))
    ...
    for i, entry in enumerate(entries_sorted):
        ...
        timeline[entry_min:next_min] = [entry["severity"]] * (next_min - entry_min)
    ...
    if universal_start_min is None:
        post_3am_entries = [
            ...
            if (parse_datetime_safe(e["time"]) - datetime.combine(selected_date, datetime.min.time())).seconds // 60 > 3*60
        ]
        if post_3am_entries:
...
def plot_timeline_matplotlib(timeline, symptom, fig_height=1):
    ...
    xticklabels = [(h % 12) or 12 for h in range(len(xticks))]  # 0 → 12, 1 → 1, ...
...
# -----------------------------
# Standard Symptom Order (from all_lists.json)
# -----------------------------
ALL_LISTS_FILE = DATA_FOLDER / "all_lists.json"
if ALL_LISTS_FILE.exists():
    with open(ALL_LISTS_FILE, "r", encoding="utf-8") as f:
        all_lists = json.load(f)
    STANDARD_SYMPTOMS = all_lists.get("symptoms", [])
else:
    st.error("all_lists.json not found!")
    STANDARD_SYMPTOMS = []
...
## -----------------------------
# Streamlit App - Symptoms Section
# -----------------------------
with st.expander("Symptoms", expanded=False):
    ...
    if "loaded_date" not in st.session_state or st.session_state.loaded_date != selected_date:
        ...
        for symptom in STANDARD_SYMPTOMS:
            entries_prev = [e for e in prev_entries if e["item"] == symptom]
            if entries_prev:
                entries_prev_sorted = sorted(entries_prev, key=lambda e: parse_datetime_safe(e["time"]))
                prev_last_entry[symptom] = entries_prev_sorted[-1]["severity"]
            else:
        ...
        for symptom in STANDARD_SYMPTOMS:
            entries = [e for e in symptom_entries if e["item"] == symptom]
    ...
    for symptom in STANDARD_SYMPTOMS:
        ...
        # 🔑 Only one plot per symptom (always above editor)
        ...
        if st.session_state.get(f"expander_{symptom}", False):
            ...
            # 🔑 Direct save replaces old timeline (force re-render immediately)
            if st.button(f"Save Change - {symptom}", key=f"save_change_{symptom}"):
                ...
                st.session_state[f"expander_{symptom}"] = False   # 👈 collapse after save
                st.rerun()
    ...
    # -----------------------------
    # Display Event Timeline (within Symptoms expander)
    # -----------------------------
    st.markdown("### Event Timeline")

    # Extract all entries from symptom_entries where category == "Event"
    event_entries = [
    ...
    if st.button("✅ Validate & Upload Symptoms Only"):
        ...
        # --- Preserve existing event entries
        event_entries = [
            e for e in data.get("symptom_entries", [])
            if e.get("category", "").lower() == "event"
        ]

        # --- Merge symptoms + events back into symptom_entries
        ...
        if "validated_entries" not in data or not data["validated_entries"]:
            ...
            # Only update symptoms_valid, preserve other keys
            data["validated_entries"][0]["symptoms_valid"] = "true"

        # Save updated symptom timelines + preserved events
...
# Load conditions from all_lists.json
ALL_LISTS_FILE = DATA_FOLDER / "all_lists.json"
if ALL_LISTS_FILE.exists():
    with open(ALL_LISTS_FILE, "r", encoding="utf-8") as f:
        all_lists = json.load(f)
    CONDITION_OPTIONS = all_lists.get("conditions", [])
else:
    st.error("all_lists.json not found!")
    CONDITION_OPTIONS = []

def merge_activity_sessions(entries, merge_gap_minutes=15):
    """Merge activity sessions (category=Activities with status Start/End) if within merge_gap_minutes.
    Start without End will appear in red.
    """
    display_list = []
    merged_sessions = []
        ...
        for e in item_entries:
            ...
            if status == "Start":
                if current_start is None:
                    current_start = time
                    current_end = time   # initialize both
                else:
...
with st.expander("Conditions & Activities", expanded=False):
    ...
    # -----------------------------
    # Display Chronological List with merged activity sessions
    # -----------------------------
    ...
    # -----------------------------
    # Save & Validate (merged activity sessions)
    # -----------------------------
    if st.button("✅ Save & Validate Conditions"):
        data = st.session_state.data

        # Keep non-activity Start/End entries unchanged
        new_entries = [
            e for e in st.session_state.conditions
            if not (e.get("category") == "Activities" and e.get("status") in ("Start", "End"))
        ]

        # Add merged activity sessions, keeping item/status separate
        ...
        # Save back to the data dict
        data["condition_entries"] = new_entries

        # Update validated_entries key
        if "validated_entries" not in data or not data["validated_entries"]:
            data["validated_entries"] = [{
...
with st.expander("Reproductive", expanded=False):
    ...
    # -----------------------------
    # Display Existing Reproductive Entries
    # -----------------------------
    ...
    if existing_repro:
        ...
        for key, value in existing_repro.items():
            if value is not False:
                # Combine checkbox label with severity if applicable
    ...
    if st.button("✅ Save & Validate Reproductive"):
        ...
        # Build the reproductive_entries dict
        ...
        # Only include severity if the checkbox is checked
        ...
        # Add or overwrite reproductive_entries top key
